<template>
    <div class="post__item">
        <div class="post__left">
            <nuxt-link :to="'/' + post.category.alias + '/' + post.alias" v-if="post.thumbnail"  @click.native="addVisit(post.id)">
                <img v-lazy="post.thumbnail" :alt="post.title" >
            </nuxt-link>
        </div>
        <div class="post__right">
            <div class="post__category">
                <nuxt-link :to="'/'+ post.category.alias">{{post.category.title}}</nuxt-link>
            </div>
            <h2 class="post__title">
                <nuxt-link :to="'/' + post.category.alias + '/' + post.alias" @click.native="addVisit(post.id)">{{post.title}}</nuxt-link>
            </h2>
            <div class="meta">
