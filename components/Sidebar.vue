    ...
    <aside class="sidebar">
        ...
        <div class="sidebar__post">
            <div v-for="post in getPosts" :key="post.id" class="post__items">
                <h3><nuxt-link :to="'/' + post.category.alias + '/' + post.alias">{{post.title}}</nuxt-link></h3>
        ...
        <div class="comments"  v-if="getComments.length">
            <div class="comment" v-for="com in getComments" :key="com.id">
                <nuxt-link :to="'/' + com.post.category.alias + '/' + com.post.alias + '#comment' + com.id">{{com.text}}</nuxt-link>
            </div>
        </div>
