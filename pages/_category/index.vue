...
export default {
  ...
  asyncData ({ params, query,  error }) {
    ...
    return axios.get('https://api.mirror-universe.ru/posts/' + params.category + '?page='+page)
      .then((res) => {
        return { posts: res.data.posts, count: res.data.count, category: res.data.category }
      })
      .catch((e) => {
  ...
  methods: {
        ...
        changePage(num) {
            axios.get('https://api.mirror-universe.ru/posts/'+this.category.alias+'?page='+num).then((data) => {
