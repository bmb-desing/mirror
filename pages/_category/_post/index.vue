                ...
                <li v-for="(item) in comments" :key="item.id" :id="'comment'+item.id" class="comment__item">
                    <div class="comment__header">
                        <div class="comment__left">
                            Коментарий <a :href="'#comment' + item.id" :name="'comment' + item.id">#{{item.id}}</a> от {{item.name}} {{dateForHumans(item.createdAt)}}
                        </div>
...
<script>
    ...
    export default {
        ...
        asyncData ({params ,error }) {
            return axios.get('https://api.mirror-universe.ru/posts/'+params.category+'/'+params.post).then((res) => {
                return {post: res.data, comments: res.data.comments}
            }).catch((e) => {
        ...
        methods: {
            ...
            getRat(result) {
                const vm = this
                setTimeout( function () {
                    axios.get('https://api.mirror-universe.ru/posts/'+vm.post.category.alias+'/'+vm.post.alias).then((res) => {
                        vm.post.rating = res.data.rating
                        vm.post.ratingCount = res.data.ratingCount
                    }).catch((e) => {
                        console.log(e)
                    })
                }, 200)
            ...
            submitForm() {
                return axios.post('https://api.mirror-universe.ru/comments/add', {
                    name: this.form.name,
